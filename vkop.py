...
try:
    ...
    prev = pd.read_excel("C:\\Users\\novikov.rn\\Desktop\\VKOPatel\\logs.xlsx").to_dict()
    ...
    if corr:
        ...
        pd.DataFrame(df).to_excel("C:\\Users\\novikov.rn\\Desktop\\VKOPatel\\logs.xlsx", index=False)
    elif uncorr:
        df = pd.read_excel("C:\\Users\\novikov.rn\\Desktop\\VKOPatel\\logs.xlsx").to_dict()
        df = {key: [df[key][i] for i in df[key].keys()] for key in df.keys()}
        df['Номенклатура'].append(f"{st.session_state.VKOP}-{item[0]}-{st.session_state.climate}")
